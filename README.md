...
| `MAX_UPLOAD_MB` | Largest accepted upload, default 25. Set to `4` on Vercel |
| `STORAGE_DRIVER` | `local` or `supabase`. Defaults to following `DATA_DRIVER` |
...
Two hosts, and the choice is really about where the data lives.

| | Vercel + Supabase | Render |
| --- | --- | --- |
| Data | Real PostgreSQL, durable | File on the instance's disk |
| Cold start | About a second | About a minute on the free plan |
...
```bash
openssl rand -hex 32     # this is AUTH_SECRET
```
...
| `MAX_UPLOAD_MB` | `4` — see below |

**Uploads are capped at 4.5MB on Vercel.** That is the platform's limit on a
request body, not the product's, and it applies before any application code
runs. Setting `MAX_UPLOAD_MB=4` turns a confusing platform failure into a plain
"files must be 4MB or smaller" message. Lifting it properly means uploading
straight from the browser to Supabase Storage with a signed URL, which this
version does not do.

### Know what you are publishing

The demo is deliberately open: anyone with the URL can sign in with one click as
the borrower, the lender **or the administrator**, and can create an account of
their own. That is right for fictional data and wrong for anything else. Never
put a real deal, a real document or a real lender's terms into a deployment
configured this way. Setting `SEED_DEMO_DATA=false` removes both the seeded data
and one-click sign-in.

### What has and has not been verified

The Supabase data path is tested, not assumed. Both migrations were applied to a
real PostgreSQL 16, PostgREST was run in front of it with the same 1000-row
ceiling hosted Supabase applies by default, and the driver was exercised against
it: filters, ordering, pagination, counts, `updated_at` triggers and the
append-only guards. The full demo seed was then loaded through the Supabase
driver and produced row-for-row identical counts to the local driver, and the
application itself was walked end to end against PostgreSQL.

Supabase **Storage** is the exception. The local stack has no object store, so
document upload and signed-URL download were exercised against the filesystem
driver rather than Supabase's. Upload one document after deploying and confirm
it opens — that is the one path still to prove.
