...
**Showing it to someone?** Use `npm run demo` instead. It builds first and then
serves, so pages load in milliseconds rather than compiling on demand — the
difference between a first page load of about fifteen milliseconds and about a
minute — and the development toolbar stays out of your screenshots.
...
| `EmailTransport` | `services/email.ts` | Logs to the console; Resend when configured |
...
Showing the product to someone should not require them to install anything.
`render.yaml` in the repository root deploys a public, shareable demo from
GitHub with no terminal involved:

1. Push this branch to GitHub.
2. At [render.com](https://render.com), choose **New → Blueprint** and pick this
   repository. Render reads `render.yaml` and fills in the rest.
3. Click **Apply**. First build takes roughly five minutes.

You get a URL like `carecapital-exchange.onrender.com`. `AUTH_SECRET` is
generated by Render on first deploy, so no secret is committed or copied by
hand. Nothing else needs configuring — the database seeds itself on the first
request and the login page signs visitors in with one click.

**Know what you are publishing.** The demo is deliberately open: anyone with the
URL can sign in with one click as the borrower, the lender **or the
administrator**, and can create an account of their own. That is right for
fictional data and wrong for anything else. Never put a real deal, a real
document or a real lender's terms into a deployment configured this way. If you
ever need the deployment closed, set `SEED_DEMO_DATA=false`, which removes both
the seeded data and one-click sign-in.

On Render's free plan the instance sleeps after inactivity and its disk is
reset when it wakes, so demo data returns to its seeded state and the first
request after a sleep takes around a minute. Both are fine for a demo you walk
someone through; neither is fine for a demo you hand out and expect to hold
state.

### Making demo data durable

Move the service to a paid instance and attach a disk, then point the store at
it by setting `CCX_DATA_DIR` to the mount path. The instance stops sleeping and
anything created during a demo survives a restart. This is still the local
file driver — durable enough to demonstrate, not a production database. For
that, see below.
...
For real transactions, as opposed to the demo above:
...
**Do not run the local driver against real deals.** It stores everything in a
file on the instance's disk, which is neither durable nor shared across
instances, and the demo configuration above leaves one-click sign-in open to
anyone with the URL. It is the right choice for a demonstration and the wrong
one for anything else. The driver refuses to start without `AUTH_SECRET` when
`NODE_ENV=production`.

The Supabase driver ships complete — schema, row-level security and storage
policies — but has not yet been exercised against a live Supabase project.
Budget time to verify it before trusting it with real data.
